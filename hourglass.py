...
for line in lines:
    ...
    fred.append(float(line.split(' ')[0]))
    strain.append(float(line.split(' ')[1]))
plt.plot(fred,strain,label='Q3 delay(2)')
#3(Q3noDelay)
...
for line in lines:
    ...
    fred.append(float(line.split(' ')[0]))
    strain.append(float(line.split(' ')[1]))
plt.plot(fred,strain,label='Q3 noDelay(3)')
...
#plotting the observation and simulations at once
plt.plot(fred,strain,label='observation')
plt.legend()
plt.axvline(x=10^(-4))
plt.savefig('plots/observations_%1.1f.eps'%(error*100),format='eps',dpi=1000)
plt.close()
...
f = open('outputs/LStrain%1.1f.txt'%(error*100),'w')
f.writelines(lines)
f.close()
...
plt.savefig('plots/LStrain%1.1f.eps'%(error*100),format='eps',dpi=1000)
plt.close()

###Bayesian priors
###(1) number of detections
###(2) distribution of detections
import matplotlib.pyplot as plt
